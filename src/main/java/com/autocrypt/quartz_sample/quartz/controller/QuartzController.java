package com.autocrypt.quartz_sample.quartz.controller;

import com.autocrypt.quartz_sample.quartz.controller.requestDTO.*;
import com.autocrypt.quartz_sample.quartz.service.QuartzSchedulingService;
...
public class QuartzController {
    ...
    private static Class<? extends Job> getJobClass(String createCronJobRequest) throws ClassNotFoundException {
        return (Class<? extends Job>) Class.forName("com.autocrypt.quartz_sample.quartz.jobs." + createCronJobRequest);
    }
    ...
    // 7. 스케줄 조회 - Trigger 조회
    @GetMapping("/trigger")
    public ResponseEntity<Trigger> getTrigger(@RequestParam String jobName, @RequestParam String jobGroup, @RequestParam boolean isOneTimeJob) throws Exception {
        Trigger trigger = quartzSchedulingService.getTrigger(jobName, jobGroup, isOneTimeJob);
